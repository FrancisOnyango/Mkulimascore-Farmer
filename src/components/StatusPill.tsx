...
export function StatusPill({ label, tone = 'neutral' }: { label: string; tone?: Tone }) {
  const palette = {
    good: { bg: colors.brandSoft, fg: colors.brandDark },
    verified: { bg: '#E8F3ED', fg: colors.success },
    attention: { bg: colors.warm, fg: colors.warmInk },
    neutral: { bg: '#EDF0ED', fg: colors.muted },
    danger: { bg: colors.claySoft, fg: colors.danger }
  }[tone];

  return (
    <View accessibilityRole="text" accessibilityLabel={label} style={[styles.pill, { backgroundColor: palette.bg }]}>
